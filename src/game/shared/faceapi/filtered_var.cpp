...
{	
	...
	if(m_scale1) 
	{
		scale *= m_scale1->GetFloat();
	}
	if(m_scale2)
	{
		scale *= m_scale2->GetFloat();
	}
	value *= scale;
...
void Fadable::EnableFading(TunableVar *duration)
{ 
	m_easeDurationOnDropout = duration;
}
...
float Fadable::FadeIn(float value, float now)
{
	...
	if(m_fadeoutTime != OFF)
	{
		m_fadeinDuration = 
				...
				// how much time the fade out required
